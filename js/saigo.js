...
window.sendMouse = setInterval(() => {
    let buf = window.buffers.mousePosition(window.playerPos.x, window.playerPos.y);
    if (window.user.startedBots && window.user.isAlive) window.connection.send(buf);
    window.checkPlayer();
}, 50);
...
function setGUI() {
    setKeysEvents();
    console.log(123);
    $('.navigation').append(`<div class="tab material-button" onclick='setTimeout(()=>{document.getElementById("botSettings").classList.add("active")}, 100);' category="botSettings" style="position: relative; overflow: hidden;"><i class="material-icons">android</i> Bots</div>`);
$('.options-list').append(`
<div id="botSettings" category="bots">
    ...
    <div id="settings" class="inputs-menu"></div>
    <div id="hotkeys" style="display:none;" class="inputs-menu ps ps--theme_default"></div>
    <div id="botPanel"></div>
</div>`)
...
<div id="settings" class="inputs-menu active"></div>
    ...
    document.getElementById('botPanel').innerHTML = `
...
function setGUIEvents() {
    ...
    document.getElementById('startBots').addEventListener('click', () => {
        if (window.game.url && window.game.protocolVersion && window.game.clientVersion && !window.user.startedBots) {
			this.partytoken = window.target.split("party_id=")[1];
			if (this.partytoken!="" && this.partytoken!=null){
				if (window.bots.name && window.bots.amount && !document.getElementById('socialLoginContainer')) window.connection.send(window.buffers.startBots(window.target, window.game.protocolVersion, window.game.clientVersion, window.user.isAlive, window.unescape(window.encodeURIComponent(window.bots.name)), window.bots.amount))
